...
static int modeset_initialize()
{
	...
	bool shouldCloseOnErr = false;

	card = "/dev/dri/card0";
	fprintf(stderr, "using card '%s'\n", card);

	/* open the DRM device */
	ret = modeset_open(&modeset_fd, card);
	if (ret)
		goto out_return;

	shouldCloseOnErr = true;	// If there's an error below, close the fd

	/* prepare all connectors and CRTCs */
	ret = modeset_prepare(modeset_fd);
	if (ret)
		goto out_return;

	/* perform actual modesetting on each found connector+CRTC */
	for (iter = modeset_list; iter; iter = iter->next) {
		iter->saved_crtc = drmModeGetCrtc(modeset_fd, iter->crtc);
		buf = &iter->bufs[iter->front_buf];
		ret = drmModeSetCrtc(modeset_fd, iter->crtc, buf->fb, 0, 0,
